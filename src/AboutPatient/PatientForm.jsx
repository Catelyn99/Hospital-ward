...
const PatientForm = (props) => {
  ...
  const saveInfo = (info) => {
    dispatch({
      type: WardTypes.SaveInfo,
      payload: { roomId: props.roomId, info: info },
    });
  };
  ...
  return ReactDOM.createPortal(
    <form className={styled.form} onSubmit={saveForm}>
      <CloseIcon
        ...
        onClick={() => props.closePatientInfo(props.patient.id)}
      />
      {props.patient.name === "" ? (
        <div className={styled.headerBed}>
          Dodaj pacjenta - łóżko {props.patient.id}
        </div>
      ) : (
        <div className={styled.headerBed}>
          Edytuj pacjenta - łóżko {props.patient.id}
        </div>
      )}
      <label>
        Imię i nazwisko:
      ...
      <input
        className={`${styled.buttons} ${styled.submit}`}
        type="submit"
        value="ZAPISZ"
      />
    </form>,
